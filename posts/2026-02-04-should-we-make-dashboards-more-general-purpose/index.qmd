...
You might see this as an issue, but think what can we do - we have many different metrics that we want to track, over different slices of our data. I'm going to persuade you that the answer is not to build a new dashboard for each new question, but instead create a general purpose dashboard that you build upon for each new question.
...
If you're not persuaded yet with this concept, then let me list you some further advantages of moving toward general purpose analytics dashboards:

* The operations data that you are analysing usually falls into one of two bucket of being used for a long time (the whole business lifetime usually) or a very short time (for a specific one off task/project). General purpose dashboards are for the former. Since the analytics dashboard will be used for such a long period of time, getting the business logic right is the most important thing.
...
Your general-purpose analytics dashboard should have three tabs for the three core types of analytical questions:
1. Time series tab (how is this changing time?)
2. Period performance tab (what does typical behaviour look like?)
3. Aggregate tab (What is the overall performance?)

#### Time series tab

This tab is used to answer questions about how has metric X changed over time. You choose your metric, what time interval you are grouping your data by (e.g. hour, day, month), what variables you want to filtered to certain values, if you want the lines to be grouped by a specific variable (or combination of variables) and if you want faceting (multiple graphs). There is also a table that shows this data, with an export to CSV option.
